...
----------------------------------------------------
--Въпрос 3:От следващата таблица напишете SQL заявка, за да намерите онези служители, чиято заплата е повече от заплатата на Steven. Върнете пълна информация за служителите.
----------------------------------------------------

use master
go
DROP DATABASE IF EXISTS въпрос_3;
go
create database въпрос_3
go
use въпрос_3
go
...
SELECT FirstName, LastName from EMPLOYEES where Salary > (select Salary as StSalary from EMPLOYEES where FirstName = 'Steven');
----------------------------------
