...
TEST_CASE("Worker sent before back_ of worker at same time") {
    TestStore store;
    store.init(5,0,0);
    store.addClients({
                             Client{0, 20, 0, 50},
    ...
    SECTION("First event") {
        store.advanceTo(0);
        INFO("Worker sent for bananas at min 0");
        REQUIRE(store.log.size() == 1);
        REQUIRE(LastEvent().type == StoreEvent::WorkerSend);
        REQUIRE(LastEvent().minute == 0);
    }
    ...
    SECTION("Last event") {
        store.advanceTo(60);
        INFO("Worker sent before worker back_");
...
TEST_CASE("No workers should not generate events") {
    TestStore store;
    store.init(0,0,0);
    store.addClients({
                             Client{0, 20, 0, 50},
    ...
    SECTION("Sent no workers") {
        INFO("No workers sent and two clients depart")
        store.advanceTo(80);
        REQUIRE(store.log.size() == 2);
    }

    SECTION("Events type") {
        INFO("Check logs type is client depart")
        store.advanceTo(80);
        REQUIRE(store.log[0].type == StoreEvent::ClientDepart);
        REQUIRE(store.log[1].type == StoreEvent::ClientDepart);
    }
